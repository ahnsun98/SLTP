...
::mediapipe::Status RunMPPGraph() {
    ...
    /*------------------*/
    LOG(INFO) << "Prepare writing landmark.";
    ...
    //output file path 
    str="/home/pi/tf_pi/Translated/Landmark/"+dir_name+video_fname+".txt";
    LOG(INFO) << str;
    /*------------------*/
    ...
    while (grab_frames) {
        ...
        for (const auto& hand_landmarks : multi_hand_landmarks) {
            for (const auto& landmark : hand_landmarks.landmark()) {
                Q.push(static_cast<float>(landmark.x()));
                Q.push(static_cast<float>(landmark.y()));
            }
        }
        /*------------------*/
        /*
    ...
    if(out.is_open()) LOG(INFO) << "Open file.";
    else LOG(INFO) << "Cannot open file." << strerror(errno);
    for(i=0; i<Q.size(); i++){
        out << Q.pop() << ' ' << Q.pop() << ' ';
    }
    out.close();
