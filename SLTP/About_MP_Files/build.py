# -*- coding: utf-8 -*-
'''run in mediapipe dir'''
import os
import sys
import argparse

def main(input_data_path,output_data_path):
    comp='bazel build -c opt --copt -DMESA_EGL_NO_X11_HEADERS --copt -DEGL_NO_X11   mediapipe/examples/desktop/multi_hand_tracking:multi_hand_tracking_gpu'
    #명령어 컴파일
    ...
    if not(os.path.isdir(output_data_path+"Landmark/")):
        os.mkdir(output_data_path+"Landmark/")
    for file in listfile:
        ...
        if not(os.path.isdir(output_data_path+"Landmark/"+word)):
            os.mkdir(output_data_path+"Landmark/"+word)
        os.system(comp)
